...
<!--      <input type="checkbox" :checked="judgeWhetherAllAreSelected" @click="toggleAllStates($event)" />-->
    ...
    <button class="btn btn-danger" @click="clearComplicatedTodos">清除已完成任务</button>
  </div>
</template>
...
export default {
  name: "Footer",
  props: ["todos","clearComplicatedTodos","toggleAllStates"],
  computed:{
    computeComplicated(){
    ...
    judgeWhetherAllAreSelected(){
      console.log(this.todos.every(todo => todo.complicated))
